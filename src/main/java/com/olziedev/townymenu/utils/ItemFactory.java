...
public interface ItemFactory {
    ...
    default ItemStack createItem(ConfigurationSection section, Function<String, String> nameReplacements, Function<List<String>, List<String>> loreReplacements, OfflinePlayer offlinePlayer) {
        if (section == null) return null; // If the section is null, return null.
        ...
        ItemStack itemStack = new ItemStack(material, section.getInt("amount", 1), (short) section.getInt("data")); // Create the item stack.
        ItemMeta im = itemStack.getItemMeta(); // Get the item meta.
        if (!(im instanceof SkullMeta skullMeta)) { // If the item meta is not a skull meta, return.
            return addBaseItemMeta(itemStack, im, section, nameReplacements, loreReplacements); // Add the base item meta.
        }
        skullMeta.setOwningPlayer(offlinePlayer); // Set the owner of the skull.
        return addBaseItemMeta(itemStack, skullMeta, section, nameReplacements, loreReplacements); // Add the base item meta.
    }
